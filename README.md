...
공공조달 시장 진입을 고려하는 소상공인·스타트업·창업 상담가를 위한
공공수요 기반 창업 아이템·입지/물류 거점 분석 서비스입니다.

나라장터 입찰공고를 지역별 공공수요 신호로 해석해, 예비창업자·납품업체·물류사가 수요 패턴을 데이터 근거로 탐색합니다.
...
1. **전국 지도**에서 시도를 클릭해 지역 분석 흐름 확인
2. **시도/자치구 드릴다운**에서 자치구 단위 수요 비교
3. **지역 비교**에서 두 지역의 공공수요·소비층 적합도 비교
4. **AI 해석 5종**에서 정량 지표를 사람이 이해하기 쉬운 설명으로 확인
5. **물류 거점 분석**에서 입지·거점 후보 비교
...
| 요건 | 구현 |
|---|---|
| 공공데이터 API 활용 | ✅ 조달청(입찰·계약·발주계획·MAS) + aT 학교급식 + 행안부 인구 + 소상공인 상권 + 국토부 물류창고 + KOSIS 생존율 — 6개 기관 / 9개 데이터소스 실결합 |
