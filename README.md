...
> 전국 지도에서 시도를 클릭하면 해당 지역 분석이 바로 열립니다.

| 탭 | 설명 |
|---|---|
| 🌏 전국 지도 | 시도 클릭 → 시군구 드릴다운, 공공수요 히트맵 |
| 🔍 사업 유형 검색 | 업종·품목으로 전국 수요 지역 검색 |
| 🗺️ 지역 분석 | 시군구 선택 → 품목별 수요 점수 + AI 해석 |
| 📦 품목 분석 | 품목군 선택 → 전국 수요 상위 지역 |
| ⚖️ 자치구 비교 | 두 지역 나란히 비교 + 파이차트 + AI 해석 |
| 👥 소비층 적합도 | 연령 인구 분포 기반 소비층 매칭 지역 |
| 🏪 경쟁 분석 | 소상공인 점포 밀도 기반 업종별 경쟁 포화도 |
| 🚚 물류 거점 분석 | 전국 수요 분포 기반 1·2·3티어 거점 후보 + AI 전략 |
| 🔬 분석 근거 데이터 | 수집 원천 데이터 직접 조회 |
| 📋 프로젝트 개요 | 데이터 규모·수집 범위·비즈니스 모델 요약 |
