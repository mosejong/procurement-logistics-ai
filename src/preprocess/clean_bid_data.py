...
ITEM_CATEGORY_KEYWORDS = {
    ...
    # 급식/식품을 교육/교구보다 먼저 체크 — "학교급식" 등이 교육으로 오분류되는 것을 방지
    "급식/식품": ["급식", "식자재", "식품", "부식", "식단", "쌀", "김치", "수산물", "축산물", "농산물", "채소", "과일", "육류", "두부", "반찬", "도시락", "배식"],
...
def clean_bid_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 교정 순서: ① 해당 시/도 district로 재추출 → ② 없으면 시/도명 자체를 district로 사용.
    if "_source_city" in df.columns:
