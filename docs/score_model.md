...
```
hub_score =
    _bids_score  × 0.40   ← 품목군 내 공고수 (전국 min-max)
  + _amt_score   × 0.25   ← 품목군 내 발주금액 (전국 min-max)
  + _cat_score   × 0.15   ← 품목 다양성 (품목수 기반)
  + _infra_score × 0.20   ← 국토부 물류창고 인프라 (경기도 기준 0~100)
```
...
| 지표 | 가중치 | 설계 이유 |
|---|---|---|
| 공고수 | 40% | 반복 발주 강도 = 물류 거점 안정성의 가장 강한 신호 |
| 발주금액 | 25% | 물류 처리 규모 반영. 단발 고액 왜곡 방지를 위해 25% 제한 |
| 품목 다양성 | 15% | 다품종 거점 가능성. 단일 품목 의존 리스크 보정 |
| 물류창고 인프라 | 20% | 수요가 있어도 창고 없으면 거점 운영 불가. 급식은 냉장·당일납품 특성상 창고 위치가 수익에 직결 |

> ⚠️ 물류창고 인프라는 국토부 물류창고업 등록정보 **수집 시점 기준**입니다. 실시간 운영 현황이 아니며, 공개 등록현황 기반 인프라 참고 지표입니다. 운영 단계에서는 연 1~2회 갱신 방식으로 설계 가능.
